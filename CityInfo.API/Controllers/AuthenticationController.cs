...
namespace CityInfo.API.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        {
            var user = ValidateUserCredetials(authenticationRequestBody.UserName, authenticationRequestBody.Password);
            if (user == null)
            {
                return Unauthorized();
            }
            var secrutiryKey = new SymmetricSecurityKey(Convert.FromBase64String(_configuration["Authentication:SecretForKey"]));
            var signingCredentials = new SigningCredentials(secrutiryKey, SecurityAlgorithms.HmacSha256);
